...
def train_inequality():
    ...
    for lr_exp in range(1, LR_NUM+1):

        for rep in range(REPS):

            print('\n-- Starting: lr 10^-%d, rep #%d --\n'%(lr_exp,rep))
            ...
            for activation in NonLinearity:
                ...
                sgd = SGD(model.parameters(), lr=10**-(lr_exp+1))
            ...
            pyplot.title('LR: 10^-%d; Repetition #%d'%(lr_exp,rep))

            if not os.path.isdir("plots"):
                os.mkdir("plots")
            pyplot.savefig('plots/inequality_%d_%d.png'%(lr_exp,rep), bbox_inches='tight')
            pyplot.clf()
    ...
    for lr_exp in range(1, LR_NUM+1):
        ...
        for rep in range(REPS):
            ...
            for recurrence in RecType:
                ...
                sgd = SGD(model.parameters(), lr=10**-lr_exp)
                loss_fn = CrossEntropyLoss()
                all_epoch = EPOCHS
                accuracies = [0]*all_epoch
            ...
            if not os.path.isdir("plots"):
                os.mkdir("plots")
            pyplot.savefig('plots/recurrence_%d_%d.png'%(lr_exp,rep), bbox_inches='tight')
            pyplot.clf()
